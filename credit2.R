...
Take_input1=function()
{
  Year =readline("Enter year:")
  Month =readline("Enter month:")
  Product =readline("Enter the Product:")
  filter_1=filter(customer1,customer1$Year==Year&customer1$Month==Month&customer1$Product==Product)
  sort1=filter_1 %>% dplyr::group_by(Customer,City,Product)%>%summarise(sum(customer1$Amount))
...
sort3=filter_2%>%dplyr::group_by(Customer,City,Product)%>%summarise(sum(customer1$Amount))
